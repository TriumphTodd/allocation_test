...
This script will:
 ...
 + perform the expense allocations from the cost centers to the
   accounts based on their account balances
...
MAP_FILE = 'mapccs.xlsx'
CALC_FILE = 'calculations.xlsx'
RECON_FILE = 'recon.xlsx'

# Remove prior output files if they exist
for x in [MAP_FILE, CALC_FILE, RECON_FILE]:
    if os.path.exists(x): 
        os.remove(x)
...
proddf = pd.merge(proddf, acctdf,
    on=['month', 'product'], \
    how='left',
    suffixes=['_map', '_accts'])
proddf = proddf.drop(columns=['channel_map'])        # just says 'all'
proddf.columns = ['month', 'cost_center', 'expense', 'maptype',
    'product', 'account_num', 'channel', 'balance']

# get ccs mapped to channels
channeldf = mapccdf[mapccdf['maptype']=='channel']
channeldf = pd.merge(channeldf, acctdf,
    on=['month', 'channel'],
    how='left',
    suffixes=['_map', '_accts'])
channeldf.reset_index()
channeldf = channeldf.drop(columns=['product_map'])        # just says 'all'
channeldf.columns = ['month', 'cost_center', 'expense', 'maptype',
    'channel', 'account_num', 'product', 'balance']

# append the two dataframnes
calcdf = pd.DataFrame()
calcdf = calcdf.append(proddf)
calcdf = calcdf.append(channeldf)
calcdf.columns = ['month', 'cost_center', 'expense', 'maptype',
    'product', 'account_num', 'channel', 'balance']
calcdf = calcdf.loc[:, ['month', 'cost_center', 'expense',
    'maptype', 'product', 'channel', 'account_num', 'balance']]      # reorder columns
...
# Export results
calcdf.to_excel(CALC_FILE, sheet_name='calculations')
...
recondf = pd.merge(expensedf, recondf,
    on=['month', 'cost_center'],
    how='left',
    suffixes=['_expense', '_calc'])
recondf['variance'] = recondf['total_cc_allocated_expense'] - recondf['expense']
recondf.set_index(['month', 'cost_center'])
recondf.to_excel(RECON_FILE)
