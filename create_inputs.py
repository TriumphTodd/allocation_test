...
# get cartesian product of months, products, channels and add a random account number and a random float for an account balance
...
# https://stackoverflow.com/questions/1953194/permutations-of-two-lists-in-python
expenses = []
for x in itertools.product(months, centers): expenses.append((x[0], x[1], float(random.randint(10**4, 10**5))))
...
with open('map.csv', 'w', newline='') as csvfile:
    ...
    # write map
    for m in map:
        writer.writerow({fieldnames[0]: m[0], fieldnames[1]: m[1], fieldnames[2]: m[2], fieldnames[3]: m[3], fieldnames[4]: m[4]})
